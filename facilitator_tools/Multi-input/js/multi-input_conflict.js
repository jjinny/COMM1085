...
// written by Nathan Gardi, 2017
		...
		function checkAnswer(index) {
			...
			if(inputblank) {
                ...
                if(!appdata.Questions[index].Fields[j].Answer.CaseSensitive) {
                	var matchfound = false;
                	for(var t=0; t < appdata.Questions[index].Fields[j].Answer.Correct.length; t++) {
                		if (thisval.toLowerCase() === appdata.Questions[index].Fields[j].Answer.Correct[t].toLowerCase()) {
                      $("#input-" + index + "-" + j).addClass("correct");
                      matchfound = true;
										}
									}
		...
		function checkAll() {
            ...
            if(!appdata.Questions[a].Fields[j].Answer.CaseSensitive) {
              var matchfound = false;
              for(var t=0; t < appdata.Questions[a].Fields[j].Answer.Correct.length; t++) {
                if (thisval.toLowerCase() === appdata.Questions[a].Fields[j].Answer.Correct[t].toLowerCase()) {
                  $("#input-" + a + "-" + j).addClass("correct");
                  matchfound = true;
                }
              }
	...
	$.getJSON( "data/"+datafilename_multi_input, function( data ) {
			  appdata = data;
		  initMI(appdata);
			});
        });
